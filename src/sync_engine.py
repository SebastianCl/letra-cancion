...
class SyncEngine:
    ...
    def set_lyrics(self, lyrics: Optional[LyricsData], duration_ms: int = 0) -> None:
        ...
        if lyrics.is_synced:
            ...
            logger.info(f"Modo de sincronización: ESTIMATED ({self._estimated_line_duration_ms}ms/línea)")
    ...
    def _get_line_at_position(self, position_ms: int) -> tuple[int, Optional[LyricLine]]:
        """
    ...
    def get_context_lines(self, before: int = 2, after: int = 2) -> list[tuple[int, LyricLine]]:
        """
...
async def main():
    ...
    def on_sync(state: SyncState):
        if state.current_line:
            print(f"\r[{state.mode.value}] Línea {state.current_line_index}: {state.current_line.text[:50]:<50}", end="")
