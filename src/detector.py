...
class MediaDetector:
    ...
    def __init__(self, target_app: Optional[str] = None):
        ...
        if not WINSDK_AVAILABLE:
            raise RuntimeError("winsdk no está disponible. Instala con: pip install winsdk")
        ...
        if session is None:
            ...
            if sessions and sessions.size > 0:
                ...
                for i in range(sessions.size):
                    ...
                    if self.target_app is None or self.target_app.lower() in source_id.lower():
                        session = s
                        break
        ...
        if session != self._current_session:
            ...
            if session:
                ...
                session.add_media_properties_changed(
                    lambda s, a: asyncio.create_task(self._on_media_properties_changed())
                )
                ...
                session.add_timeline_properties_changed(
                    lambda s, a: asyncio.create_task(self._on_timeline_properties_changed())
                )
    ...
    async def _update_playback_info(self) -> None:
        ...
        try:
            ...
            state_changed = (
                self._current_playback is None or
                self._current_playback.state != new_playback.state
            )
            ...
            if state_changed:
                ...
                logger.debug(f"Estado: {state.name}, Pos: {position_ms}ms, Dur: {duration_ms}ms")
    ...
    def is_playing(self) -> bool:
        ...
        return (
            self._current_playback is not None and
            self._current_playback.state == PlayerState.PLAYING
        )
    ...
    try:
        ...
        while datetime.now() < end_time:
            ...
            if detector.current_playback and detector.is_playing:
                ...
                print(f"\r   Posición: {pos//1000}s / {dur//1000}s   ", end="", flush=True)
