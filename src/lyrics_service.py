...
class LyricsCache:
    ...
    def save(self, artist: str, title: str, lyrics_data: LyricsData) -> None:
        ...
        """
        try:
            cache_path = self._get_cache_path(artist, title, synced=lyrics_data.is_synced)
...
class NetEaseProvider:
    ...
    async def _get_lyrics(self, song_id: int, duration_ms: Optional[int] = None) -> Optional[LyricsData]:
...
class LyricsService:
    ...
    async def initialize(self) -> None:
        ...
        logger.info("LyricsService inicializado con proveedores: " + 
            ...
            try:
                ...
                if result:
                    ...
                    if prefer_synced and not result.is_synced:
                        logger.debug(f"{provider_name}: encontró solo letra plana, continuando...")
                        continue
                    
                    # Guardar en caché
                    self.cache.save(artist, title, result)
                    
                    logger.info(f"Letras encontradas en {provider_name} para: {artist} - {title}")
                    return LyricsSearchResult(
                        lyrics_data=result,
