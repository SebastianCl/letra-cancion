...
class LyricsData:
    ...
    def get_context_lines(self, current_idx: int, before: int = 2, after: int = 2) -> list[tuple[int, LyricLine]]:
        """
...
class LRCParser:
    ...
    def parse_plain_lyrics(cls, plain_text: str, duration_ms: int = 0) -> LyricsData:
        ...
        """
        lines_text = [line.strip() for line in plain_text.strip().split('\n') if line.strip()]
        ...
        if duration_ms > 0 and len(lines_text) > 1:
            ...
            usable_duration = max(duration_ms - start_offset - end_offset, duration_ms // 2)
